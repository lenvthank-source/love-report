...
class VedAstroService:
    ...
    def get_dasa_at_range(self, lat: float, lon: float, birth_time_str: str, birth_date_str: str, birth_offset_str: str,
                          ...
                          levels: int = 2, precision_hours: int = 24) -> List[Dict[str, Any]]:
        endpoint = (f"Calculate/DasaAtRange/Levels/{levels}/PrecisionHours/{precision_hours}/"
                    f"Location/{lat},{lon}/Time/{birth_time_str}/{birth_date_str}/{birth_offset_str}/"
                    f"Location/{lat},{lon}/Time/{start_time_str}/{start_date_str}/{birth_offset_str}/"
                    f"Location/{lat},{lon}/Time/{end_time_str}/{end_date_str}/{birth_offset_str}/"
                    f"Ayanamsa/LAHIRI")
        res = self._make_request(endpoint)
        return res.get("DasaAtRange", [])
